...
public class BerechnePlanungBatch {
    ...
    private Integer berechnePlan(DBBatch db, Hashtable hash_plan, String kategorie_id) {
        if (debug) {
        ...
        try {
            ...
            while (!cal_akt.after(cal_end)) {
                ...
                // out.println("<p>gesamt: "+gesamte_tage+" Vergangen: "+vergangene_tage+"
                // Faktor: "+faktor+"<p>");
                // System.out.println("Kategorie_id="+kategorie_id);
                if (kategorie_id.equals("-1")) {
                    // Alle Ausgaben
                    wert_relativ = db.getPlanungAllWhere(new Integer(plan_id), faktor,
                            buildWhere(db, "ausgabe", plan_id, ""));
                    // System.out.println("Plan: "+wert_relativ);
                    summe = db.getKategorienAlleSummeWhere(formatter.format(hash_plan.get("startdatum")),
                    ...
                    if (kategorie_id.equals("-2")) {
                        ...
                        summe = db.getKategorienAlleRecursivSumme(kategorie,
                                formatter.format(hash_plan.get("startdatum")), formatter.format(cal_akt.getTime()),
                                rule, new Integer(plan_id));
                    }
                }
                prozent = 0.0;
                if (wert_relativ == 0.0) {
            ...
            if (((String) hash_plan.get("batch")).equals("ja")) {
                ...
                for (int j = 0; j < kat_aus.size(); j++) {
                    ...
                    if (db.getPlanAktuellIsInWork(((Integer) hash_plan.get("plan_id")).toString(), kategorie_id)) {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString() + " Wird gerade bearbeitet. "
                                + kategorie_id);
                        continue;
                    }
                    ...
                    {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString()
                                + " Ist schon Updated mit Kategorie " + kategorie_id);
                        continue;
                    }
                    if (checkTime(db, ((Integer) hash_plan.get("plan_id")).toString(), kategorie_id))
                    ...
                    if (db.getPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(), kategorie_id)) {
                        db.updatePlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), kategorie_id,
                                1, 0);
                    } else {
                        db.insertPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), kategorie_id,
                                1, 0);
                    }

                    Integer duration = berechnePlan(db, hash_plan, kategorie_id.toString());
                ...
                if (zuBerechnen.contains(-1)) {
                    ...
                    if (db.getPlanAktuellIsInWork(((Integer) hash_plan.get("plan_id")).toString(), -1)) {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString()
                                + " Wird gerade bearbeitet. Alle Ausgaben");
                        continue;
                    }
                    ...
                    {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString()
                                + " Ist schon Updated mit Kategorie -1");
                        continue;
                    }
                    if (checkTime(db, ((Integer) hash_plan.get("plan_id")).toString(), -1))
                    ...
                    if (db.getPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(), -1)) {
                        db.updatePlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -1, 1, 0);
                    } else {
                        db.insertPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -1, 1, 0);
                    }
                    Integer duration = berechnePlan(db, hash_plan, "-1");

                    cal_akt = Calendar.getInstance();
                    if (db.getPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(), -1)) {
                        db.updatePlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -1, 0,
                                duration);
                    } else {
                        db.insertPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -1, 0,
                ...
                if (zuBerechnen.contains(-2)) {
                    ...
                    if (db.getPlanAktuellIsInWork(((Integer) hash_plan.get("plan_id")).toString(), -2)) {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString()
                                + " Wird gerade bearbeitet. Alle Einnahen");
                        continue;
                    }
                    ...
                    {
                        System.out.println(((Integer) hash_plan.get("plan_id")).toString()
                                + " Ist schon Updated mit Kategorie -2");
                        continue;
                    }
                    if (checkTime(db, ((Integer) hash_plan.get("plan_id")).toString(), -2))
                    ...
                    if (db.getPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(), -2)) {
                        db.updatePlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -2, 1, 0);
                    } else {
                        db.insertPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -2, 1, 0);
                    }
                    Integer duration = berechnePlan(db, hash_plan, "-2");

                    cal_akt = Calendar.getInstance();
                    Timestamp timestampEnd = new Timestamp(System.currentTimeMillis());
                    int timeUsed = timestampEnd.getSeconds() - timestampBegin.getSeconds();
                    System.out.println("Dauer der Berechnung: " + timeUsed + " Sekunden");
                    if (db.getPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(), -2)) {
                        db.updatePlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -2, 0,
                                duration);
                    } else {
                        db.insertPlanAktuell(((Integer) hash_plan.get("plan_id")).toString(),
                                formatter.format(cal_akt.getTime()), formatTime.format(cal_akt.getTime()), -2, 0,
    ...
    private boolean checkTime(DBBatch db, String plan_id, Integer kategorie_id) {
        ...
        try {
            ...
            // System.out.println("cal bei start = = " +
            // formatter.format(cal_now.getTime()));
            // System.out.println("cal aus plan_aktuell = " +
            // formatter.format(cal_tmp.getTime()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void searchSub(Vector all, String kat, Integer id, Vector allkat, int tiefe) {
        for (int i = 0; i < allkat.size(); i++) {
            if (((String) ((Hashtable) allkat.elementAt(i)).get("parent")).equals(kat)) {
                searchSub(all, (String) ((Hashtable) allkat.elementAt(i)).get("name"),
                        ((Integer) ((Hashtable) allkat.elementAt(i)).get("id")), allkat, tiefe + 1);
            }
        }
        if (tiefe > 0) {
            all.addElement(id);
    ...
    String buildWhere(DB db, String mode, String plan_id, String rule) {
        ...
        for (int i = 0; i < all.size(); i++) {
            if (!db.getPlanWertNull(plan_id, (Integer) ((Hashtable) all.elementAt(i)).get("id"))) {
                ...
                // searchSub(allIds,(String)((Hashtable)all.elementAt(i)).get("name") ,
                // (Integer)((Hashtable)all.elementAt(i)).get("id"), all,0);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 4) {
            System.out.println(
                    "usage: BerechnePlanungBatch <user> <password> <connectstring,z.B: jdbc:postgresql://192.168.2.28/> all|trigger  ");
            System.exit(1);
        }
        user = args[0];
        pass = args[1];
