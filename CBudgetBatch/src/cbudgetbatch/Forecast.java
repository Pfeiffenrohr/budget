    ...
    private void getAllKategoriesWithForecast(DBBatch db) {
        ...
        for (int i = 0; i < kategories.size(); i++) {
            ...
            for (int j = 0; j < konten.size(); j++) {
                ...
                /*
                 * if (!((String) kategorie.get("name")).equals("Handy")) { continue; }
                 * 
                ...
                // Hier machen wir das Ganze 3 Mal hintereinander für jedes Jahr. Das sollte man
                ...
                // Double wertYear3 =
                // db.getKategorienAlleSummeWhere(formatter.format(calThreeYearBack.getTime()),
                // formatter.format(calTowYearBack.getTime()), where);
                ...
                // oat.setSummeGewichtet((3 * yt1.getSumOfYear() + 2 * yt2.getSumOfYear() +
                // yt3.getSumOfYear()) / 6);
                ...
                for (int k = 1; k < 366; k++) {
                    ...
                    oat.computeProzentDay(k, mapYear1.get(k), mapYear2.get(k), mapYear3.get(k),
                            yt1.getAnzOfDaysNotZero(), yt2.getAnzOfDaysNotZero(), yt3.getAnzOfDaysNotZero());
                }
                // oat.printSumProzent();
                 ...
                 * +" "+ + wertYear3 ); System.out.println("Year2" + kategorie.get("name")+ " "+
                 * konto.get("name") +" "+ + wertYear2); System.out.println("Year1" +
                 * kategorie.get("name")+ " "+ konto.get("name") +" "+ + wertYear1);
                 * System.out.println("WertMonth "+ wertMonth);
                 * System.out.println("WertUngewichtet "+ wertUngewichtet);
                 * System.out.println("WertUngewichtetMap "+ wertUngewichtet);
...

                {
                    ...
                    /*
                     * 
                     * System.out.println("Wert ohne inflation = "+wert *
                     * prozent[getMonth(calstart)] );
                     * System.out.println("Wert mit inflation = "+myWert );
                     * 
                     * System.out.println("Datum = " + (String)
                     * formatter.format(calstart.getTime())); System.out.println("Inflation = "
                     * +inflation );
                     * System.out.println("Prozentwert = "+prozent[getMonth(calstart)]);
                     * System.out.println("Inflationswert =  " + wert * prozent[getMonth(calstart)]
                     * *inflation )
                     */
