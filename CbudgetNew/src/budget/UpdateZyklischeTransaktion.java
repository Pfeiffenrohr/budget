	...
	public boolean update(DB db, PrintWriter out, Hashtable settings) {
		...
		if (db.getPlanedTransaktionen(formatter.format(cal.getTime()), planed)) {
			for (int i = 0; i < planed.size(); i++) {
				out.println("<p>");
				out.println("Meldung: Geplante Transaktion " + ((String) ((Hashtable) planed.elementAt(i)).get("name"))
						+ " liegt in der Vergangenheit und wird gelöscht");
				out.println("<p>");
			}
			meldung = true;
			...
			// TODO: Hier muss evtl geschaut werde, ob ein Enddatum vorhanden ist.
			{
...

				{
					...
					if (db.getCycleTransaktion(kor_konto)) {
						...
						out.println("<p>");
						out.println("Meldung: Konto " + db.getKontoName((Integer) kor_konto.get("konto"))
								+ " wurde angepasst");
						out.println("<p>");
